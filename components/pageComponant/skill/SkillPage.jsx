  ...
  return (
    ...
    >
      <div className=" flex items-center justify-center">
        ...
        <img src={"/assets/navIcon/skill.gif"} width={70} height={70}></img>
      </div>

      <div className=" container mx-auto flex items-center justify-center">
        <div className=" flex flex-wrap items-center max-w-2xl   gap-12 justify-center">
          {skillList.data.map((skill, index) => (
            <TooltipProvider delayDuration={80} key={skill.name}>
              <Tooltip>
                <TooltipTrigger className="">
                  <div
                    className={`w-20 h-20 ${
                      index === skillList.length - 1
                        ? "md:col-span-5 justify-self-center"
                        : ""
                    } `}
                  >
                    {" "}
                    <img
                      src={skill.img}
                      className="shadow-accent shadow-md hover:shadow-xl hover:shadow-accent "
                    ></img>
                  </div>
                </TooltipTrigger>
